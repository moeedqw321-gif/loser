...
document.addEventListener && (document.addEventListener("webkitfullscreenchange", fullscreenHandler, !1), document.addEventListener("mozfullscreenchange", fullscreenHandler, !1), document.addEventListener("fullscreenchange", fullscreenHandler, !1), document.addEventListener("MSFullscreenChange", fullscreenHandler, !1));
...
function CGfxButton(a, e, b, d) {
    ...
    this.buttonRelease = function() {
        c || (playSound("click", 1, !1), k.scaleX = 1, k.scaleY = 1, f[ON_MOUSE_UP] && f[ON_MOUSE_UP].call(g[ON_MOUSE_UP], h))
    };
    this.buttonDown = function() {
        c || (k.scaleX = .9,
...
function CMenu() {
    ...
    this._init = function() {
        ...
        qwe = new CTextButton(CANVAS_WIDTH - asd.width / 4 - 120, asd.height / 2 + 10, asd, ' ', PRIMARY_FONT, "#ffffff", 40, !0, s_oStage);
        qwe.addEventListener(ON_MOUSE_UP, function(){
			window.location = '../../users/casino';
		}, this);
        var p = s_oSpriteLibrary.getSprite("but_play");
        h = new CGfxButton(CANVAS_WIDTH / 2, CANVAS_HEIGHT - 130, p, !0);
        h.addEventListener(ON_MOUSE_UP, this._onButPlayRelease, this);
        if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) p = s_oSpriteLibrary.getSprite("audio_icon"), c = CANVAS_WIDTH - p.height / 2 - 10, f = p.height / 2 + 10, m = new CToggle(c, f, p, s_bAudioActive), m.addEventListener(ON_MOUSE_UP,
        ...
        l && !1 === inIframe() && (t = new CToggle(b, d, p, s_bFullscreen, !0), t.addEventListener(ON_MOUSE_UP, this._onFullscreenRelease, this));
    ...
    this.refreshButtonPos = function(g, h) {
        !1 !== DISABLE_SOUND_MOBILE &&
            !1 !== s_bMobile || m.setPosition(c - g, h + f);
        SHOW_CREDITS && r.setPosition(a + g, h + e);
        l && !1 === inIframe() && t.setPosition(b + g, d + h)
    };
    this.unload = function() {
        h.unload();
        h = null;
...
function CGame(a) {
    ...
    this.onPlayerSelection = function(a) {
        ...
        if (b < WIN_OCCURRENCE) {
            var b = Math.random(),
                c = 4 / (52 - g);
            "high" === a ? (d = !0, b < c && 0 !== y.length || 0 === x.length ? this._pickCard("ace") : this._pickCard("high")) : (d = !1, b < c && 0 !== y.length || 0 === v.length ? this._pickCard("ace") : this._pickCard("low"))
        } else "high" === a ? (d = !0, this._pickCard("low")) : (d = !1, this._pickCard("high"));
        z.showCard()
    };
    this.checkWin = function(a) {
        if (!d && 8 > a.getRank()) {
            ...
            (8 <= a.getRank() || 1 === a.getRank()) ? (b = 2 * f, b = new CWinText(b.toString().replace(/\B(?=(?:\d{3})+(?!\d))/g, ",") + " " + " برنده شدید", e, b), b.show(), k++, r++, t++, playSound("win", .3, !1)) : (playSound("game_over", .3, !1), b = f, b = new CWinText(TEXT_LOSE + " " + b.toString().replace(/\B(?=(?:\d{3})+(?!\d))/g, ","), e, 0), b.show(), 8 > a.getRank() ? n++ : t++)
    };
...
function CInterface() {
    ...
    this._init = function() {
        ...
        f = q.height / 2 + 10;
        h = new CGfxButton(c, f, q, !0);
        h.addEventListener(ON_MOUSE_UP, this._onExit, this);
        var F = CANVAS_WIDTH - q.width / 2 - 70;
        if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) q = s_oSpriteLibrary.getSprite("audio_icon"), b = F - 25, d = q.height / 2 + 10, g = new CToggle(b, d, q, s_bAudioActive), g.addEventListener(ON_MOUSE_UP, this._onAudioToggle,
            this);
        q = window.document;
        F = q.documentElement;
        ...
        D && !1 === inIframe() && (q = s_oSpriteLibrary.getSprite("but_fullscreen"), a = q.width / 4 + 10, e = q.height / 2 + 10, E = new CToggle(a, e, q, s_bFullscreen, !0), E.addEventListener(ON_MOUSE_UP, this._onFullscreenRelease, this));
        q = s_oSpriteLibrary.getSprite("but_bet");
        r = new CTextButton(1237, CANVAS_HEIGHT - 46, q, TEXT_ALLIN, PRIMARY_FONT, "#ffffff", 30, !1, s_oStage);
        r.setTextPosition(-3, 0);
        r.addEventListener(ON_MOUSE_UP, this._onButAllinRelease, this);
        r.enable();
        q = s_oSpriteLibrary.getSprite("but_bet");
        k = new CTextButton(295, CANVAS_HEIGHT - 46, q, TEXT_CLEARBET, PRIMARY_FONT, "#ffffff", 30, !1, s_oStage);
        ...
        u = new CGfxButton(CANVAS_WIDTH / 2 - q.height / 2 - 100, CANVAS_HEIGHT / 2 - q.height / 2 - 5, q, !1);
        u.addEventListener(ON_MOUSE_UP, this._onArrowHigh, this);
        q = s_oSpriteLibrary.getSprite("arrow_low");
        v = new CGfxButton(CANVAS_WIDTH / 2 - q.height / 2 + 235, CANVAS_HEIGHT / 2 - q.height / 2 + 85, q, !1);
        ...
        for (var G = 0; 6 > G; G++){
			...
			F[G] = new CGfxButton(450 + 50 * G, 630, q, !0);
    ...
    this.refreshButtonPos = function(k, p) {
        ...
        !1 !== DISABLE_SOUND_MOBILE && !1 !== s_bMobile || g.setPosition(b - k, d + p);
        D && !1 === inIframe() && E.setPosition(a + k, e + p)
    };
    this.unload = function() {
        D && !1 === inIframe() && E.unload();
        h.unload();
        r.unload();
        k.unload();
        u.unload();
...
function CFichesController(a, e, b) {
    ...
    this.update = function(a) {
        if (!c)
            if (f += a, f > TIME_FICHES_MOV) f = 0, c = !0, l[FICHES_END_MOV] && l[FICHES_END_MOV].call(n[FICHES_END_MOV], d,
                g);
            else {
