...
class Users extends Public_Controller {
	...
	public function register($user_aff = null){
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST' AND count($_POST) >= 1){
			...
			if(count($errors) >= 1){
				$this->message->set_message(join('<br>', $errors), 'fail', 'ثبت نام', 'users/register')->redirect();
			}else{
				if($this->formValidate()){
					$credentials = array('email' => $email, 'password' => $password, 'mobile' => '0' . $mobile, 'first_name' => $first_name, 'last_name' => $last_name);
					if($this->sentinel->validForCreation($credentials)){
						...
						if($user){
							...
							try{
								...
								$mail->Subject = 'اطلاعات حساب کاربری - BWin7.org';
								...
								$mail->isHTML(true);
								$mail->Send();
							} catch(phpmailerException $error) {} catch(Exception $error) {}
							$this->sentinel->login($user);
							$messageSMS = "ثبت نام شما با موفقیت انجام شد\n\nایمیل: " . $email . "\nکلمه عبور: " . $password . "\nتاریخ ثبت نام: " . Miladr\Jalali\jDateTime::date('l d F Y , ساعت H:i', time()) . "\n\nاز حضور شما در جمع BWin7 خرسندیم";
							file_get_contents('http://webone-sms.ir/SMSInOutBox/SendSms?username=' . SMS_USERNAME . '&password=' . SMS_PASSWORD . '&from=' . SMS_DEDICATED_NUMBER . '&to=0' . $mobile . '&text=' . urlencode($messageSMS));
							$this->message->set_message('حساب کاربری شما با موفقیت ساخته شد !', 'success', 'ثبت نام', 'dashboard')->redirect();
					   }else{
							$this->message->set_message('متاسفانه مشکلی در ثبت کاربر به وجود آمده است . لطفا چند ساعت دیگر مجددا تلاش فرمایید', 'warning', 'ثبت نام', 'users/register')->redirect();
					   }
					}
				}else{
					$this->message->set_message('متاسفانه مشکلی در ثبت کاربر به وجود آمده است . لطفا چند ساعت دیگر مجددا تلاش فرمایید', 'warning', 'ثبت نام', 'users/register')->redirect();
		...
		$this->smart->assign(array('title' => 'ثبت نام', 'action' => 'users/register/' . $user_aff));
	...
	public function sendEmailReminder($user, $Reminder){
		...
		try{
			...
			$mail->SetFrom(MAIL_SENDER_USERNAME, MAIL_SENDER_NAME);
			$mail->Subject = 'فراموشی کلمه عبور - BWin7.org';
			$mail->AltBody = 'مرورگر شما از این قابلیت پشتیبانی نمی کند . لطفا مرورگر خود را بروز رسانی کنید';
			$mail->CharSet = 'utf8';
			$mail->ContentType = 'text/html';
			$mail->MsgHTML($body);
					...
					if(count($errors) >= 1){
						...
						if($this->formValidate()){
							...
							try{
								...
								$mail->AddAddress($user->email);
								$mail->SetFrom(MAIL_SENDER_USERNAME, MAIL_SENDER_NAME);
								$mail->Subject = 'تغییر کلمه عبور - BWin7.org';
								$mail->AltBody = 'مرورگر شما از این قابلیت پشتیبانی نمی کند . لطفا مرورگر خود را بروز رسانی کنید';
								$mail->CharSet = 'utf8';
								$mail->ContentType = 'text/html';
								$mail->MsgHTML($body);
	...
	public function representation(){
		...
		$sub_count = 0;
		$my_total = Affiliate::where('user_id', $this->user->id)->get();
		if($my_total){
			$trans = Transaction::where('invoice_type', 5)->where('user_id', $this->user->id)->get();
			$sub_count = $my_total->count();
			$affilate_total = 0;
			foreach($transactions as $val){
				$affilate_total = $affilate_total + $val->price;
			}
			$this->smart->assign(array('my_total' => $my_total, 'affilate_total' => $affilate_total));
		}
        $site_name = Setting::findByCode('site_name');
		$site_name = $site_name->value;
        $affiliate_count = Setting::findByCode('affiliate_count');
		$affiliate_count = $affiliate_count->value;
		$this->smart->assign(array('sub_count' => $sub_count, 'site_name' => $site_name, 'affiliate_count' => $affiliate_count));
	...
	public function withdraw(){
		if($_SERVER['REQUEST_METHOD'] == 'POST' AND count($_POST) >= 1){
			...
			($amount <= 49999 AND !empty($amount) AND is_numeric($amount)) ? $errors[] = 'حداقل مبلغ قابل برداشت 50,000 تومان می باشد' : '';
			...
			// (date('Y-m-d', strtotime($last_withdraw->created_at)) == date('Y-m-d')) ? $errors = array('در هر روز فقط یک بار امکان درخواست برداشت جایزه از حساب وجود دارد . لطفا تا تاریخ ' . Miladr\Jalali\jDateTime::date('l d F Y ساعت H:i:s', strtotime($last_withdraw->created_at) + 86400) . ' صبر کنید و مجددا درخواست برداشت جایزه ثبت کنید') : '';
			...
			if(count($errors) >= 1){
				...
				if($this->formValidate()){
					$this->load->eloquent('withdraw');
					Withdraw::create(array('amount' => $amount, 'account_holder' => $account_holder, 'bank_name' => $bank_name, 'card_no' => $card_number, 'sheba' => $sheba, 'account_number' => $account_number, 'status' => 0, 'user_id' => $this->user->id));
					$user = $this->sentinel->getUserRepository();
					...
					if($user->update($this->user, array('cash' => $currentCash - $amount))){
						...
						try{
							...
							$mail->SetFrom(MAIL_SENDER_USERNAME, MAIL_SENDER_NAME);
							$mail->Subject = 'درخواست جایزه - BWin7.org';
							$mail->AltBody = 'مرورگر شما از این قابلیت پشتیبانی نمی کند . لطفا مرورگر خود را بروز رسانی کنید';
							$mail->CharSet = 'utf8';
							$mail->ContentType = 'text/html';
							$mail->MsgHTML($body);
			...
			$this->smart->assign(array('cash' => $this->__getUserCash(), 'withdrawList' => $withdrawList, 'action' => site_url('users/withdraw'), 'my_name' => $my_name));
