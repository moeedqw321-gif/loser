...
function prepareButtonEvents() {
    ...
    $(".inplaybtn").unbind("click").click(function() {
        var s, f, e, o,w;
		k = $(this).data("eventid");		
        if ($(".betlist ul.bet").length < 21 && ($(".alertbox").addClass("hidden"), !$(".event-" + k + " > span > div > a").hasClass("selected") && !$(this).hasClass("passive-ma"))) {
            $(".nobet").hide();
            $(".bettotal").show();
            s = new Date;
			...
			updateMultiple();
			cuntBets();
        }else if ($(".betlist ul.bet").length < 21 && ($(".alertbox").addClass("hidden"), !$(this).hasClass("selected") && !$(this).hasClass("passive-ma"))){
		var n, i, r, t, u;
        i = $.cookie("betslip");
		if ($.cookie("betslip")){
        ...
        typeof i != "undefined" && (r = JSON.parse(i), findAndRemove(r, "eventid", k), t = new Date, u = 30, t.setTime(t.getTime() + u * 6e4), $.cookie("betslip", JSON.stringify(r), {
            expires: t,
            path: "/"
        }));
		$(".betlist > [data-eventid="+ k +"]").remove();
		$(".market-box-10 > [data-runnerid="+tw+"]").removeClass("selected");
